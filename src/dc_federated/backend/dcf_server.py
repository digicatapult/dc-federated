...
class DCFServer(object):
    ...
    ----------
        ...
        key_file: str
            ...
            worker_key_pair_tool tool. If all workers are to be accepted,
            supply a file with a single line with the text in
            _constants.ALL_WORKERS_ALLOWED
    ...
    def __init__(
        ...
        self.worker_authenticator = WorkerAuthenticator(key_file)
...
class WorkerAuthenticator(object):
    """
    Helper class to authenticate workers
    ...
    key_file: str
        ...
        _constants.ALL_WORKERS_ALLOWED
    """
    def __init(self, key_file):
        lines = []
        with open(key_file, 'r') as f:
            keys = f.read().splitlines()

        if keys[0] == ALL_WORKERS_ALLOWED:
            logger.warning(f"{ALL_WORKERS_ALLOWED} is the first line in key-file - "
    ...
    def authenticate_worker(self, str_public_key, signed_message):
        """
        Tries to find a worker  a given worker using he
        ...
        try:
            ...
            logger.warning(f"Unable to authenticate worker with public key: {str_public_key}.")
            return False
        else:
            return True
